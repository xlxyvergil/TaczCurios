...
public final class EvolutionExecutor {
    ...
    public static boolean evolve(LivingEntity entity, Predicate<ItemStack> fromMatcher, Supplier<ItemStack> toStackSupplier,
        ...
        ICuriosItemHandler inv = CuriosApi.getCuriosInventory(entity).orElse(null);
        if (inv == null) return false;
        return evolve(inv, entity, fromMatcher, toStackSupplier, nbtMode, excludes, nbtMutator, postTaczChangeEvent);
    }
    ...
    private static boolean evolve(ICurioStacksHandler stacksHandler, LivingEntity entity, String slotId, Predicate<ItemStack> fromMatcher,
        ...
        for (int i = 0; i < stackHandler.getSlots(); i++) {
            ...
            if (nbtMode == NbtMode.COPY_ALL && ItemNbtHelper.hasTag(oldStack)) {
                ItemNbtHelper.setTag(newStack, ItemNbtHelper.getTag(oldStack));
            }
            if (!excludeNbtKeys.isEmpty() && ItemNbtHelper.hasTag(newStack)) {
                CompoundTag tag = ItemNbtHelper.getTag(newStack);
                for (String key : excludeNbtKeys) {
                    tag.remove(key);
                }
                ItemNbtHelper.setTag(newStack, tag);
            }
            if (nbtMutator != null) {
