 ...
 * 所有方法都内置 ModList.get().isLoaded() 检查，确保 RevelationFix 未安装时不会引发类加载错误。
 */
public final class ApollyonCompat {
     ...
     * 用 target.getHealth() 读取当前血量（Nether 下 RevelationFix 已重写此方法返回自定义健康值），
     * 再用 revelaionfix$setApollyonHealth 写入扣除后的血量（setTheData 已被 RevelationFix 的 mixin plugin 替换为实际字节码）。
     * 调用前需确保 isRevelationFixApostle 返回 true。
     */
    public static float applyDirectDamage(LivingEntity target, float damage) {
        // 用 Minecraft 的 getHealth() 而非接口的 getApollyonHealth()，后者底层 getTheData 未被 plugin 替换，永远返回 0。
