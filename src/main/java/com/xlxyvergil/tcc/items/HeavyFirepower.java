...
public class HeavyFirepower extends ItemBaseCurio {
    ...
    // 效果参数
    private static final double DAMAGE_BOOST = 1.65;       // 165%手枪伤害提升（加算）
     ...
     * 提升手枪伤害（加算）和不精准度（乘算）
     */
    private void applyHeavyFirepowerEffects(Player player) {
     ...
     * 移除重装火力效果
     */
    private void removeHeavyFirepowerEffects(Player player) {
        var attributes = player.getAttributes();
        ...
        if (slotContext.entity() instanceof Player player) {
            player.displayClientMessage(
                net.minecraft.network.chat.Component.literal(
                    "§6重装火力已装备 - 提升165%手枪伤害（加算），提高55%不精准度（乘算）"
                ),
                true
            );
        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        // 添加装备效果
        tooltip.add(Component.translatable("item.tcc.heavy_firepower.effect")
