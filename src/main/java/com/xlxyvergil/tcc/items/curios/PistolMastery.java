...
public class PistolMastery extends BaseCurioItem {
    ...
    protected void applyEffects(LivingEntity livingEntity) {
        if (GunTypeChecker.isHoldingPistol(livingEntity)) {
            double critChanceBoost = FusionUpgradeUtil.getActualValue(TaczCuriosConfig.COMMON.pistolMasteryCritChance.get(), getFusionLevel());
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        double critChanceBoost = FusionUpgradeUtil.getActualValue(TaczCuriosConfig.COMMON.pistolMasteryCritChance.get() * 100, FusionUpgradeUtil.getLevel(stack));
