...
public class WangshiDeKuqiu extends BaseCurioItem {
    ...
    /** 崩坏病等级 */
    private static final int DISEASE_AMPLIFIER = 0;
    ...
    public static void onGunHurtPost(EntityHurtByGunEvent.Post event) {
        ...
        ItemStack equipped = CurioSearchHelper.findFirstEquippedStack(attacker,
                stack -> stack.getItem() instanceof WangshiDeKuqiu);
        if (equipped.isEmpty()) {
            return;
        }
        if (!((WangshiDeKuqiu) equipped.getItem()).matchesRestriction(attacker)) {
            return;
        }
