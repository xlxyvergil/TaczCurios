...
public final class EntityConditionHelper {
    ...
    /**
     * 判断实体是否匹配指定实体键：* 匹配任意实体；#undead 等按原版实体类型 tag 判定（与原版亡灵杀手等附魔语义一致）；#namespace:tag 匹配实体类型 tag；其它值精确匹配实体类型 ID。
     */
    public static boolean matchesEntityKey(String entityKey, Entity entity) {
        ...
        if (entityKey.startsWith("#")) {
            ...
            TagKey<EntityType<?>> mobTypeTag = parseMobTypeTag(tagStr);
            if (mobTypeTag != null) {
                return entity.getType().is(mobTypeTag);
            }
    ...
    /**
     * 解析实体类型 tag，大小写不敏感；支持 undead、minecraft:undead 等形式及 arthropod/illager/water，未知名称返回 null。
     */
    @Nullable
    private static TagKey<EntityType<?>> parseMobTypeTag(String name) {
        if (name == null) return null;
        switch (name.toLowerCase()) {
            case "undead":
            case "minecraft:undead":
                return EntityTypeTags.UNDEAD;
            case "arthropod":
            case "minecraft:arthropod":
                return EntityTypeTags.ARTHROPOD;
            case "illager":
            case "minecraft:illager":
                return EntityTypeTags.ILLAGER;
            case "water":
